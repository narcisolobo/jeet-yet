...
Every recipe also carries `ownerId` (Firebase Auth uid) and `visibility` (`"private" | "public"`, default `"private"`), added starting in Phase 1 even though only one account exists until Phase 7 — this avoids a data migration and security-rule rewrite when multi-user support lands. `/share/[recipeId]` reads the recipe doc directly and checks `visibility == "public"`, so no separate denormalized "shared recipes" collection is needed.
...
- **Hosting:** Next.js needs SSR support, so plain Firebase Hosting (static-only) isn't sufficient — likely Firebase App Hosting (which supports Next.js) or Vercel. TBD.
- **Auth:** Firebase Auth, Google OAuth only (no email/password). Restricted to a single authorized account (mine) through Phase 6; Phase 7 opens sign-in to any Google account. Firestore/Storage security rules key off each recipe's `ownerId`: writes always require `request.auth.uid == ownerId`; reads are allowed for the owner or when the recipe's `visibility` is `public` (unauthenticated reads included). Comments/ratings (Phase 7) follow the same pattern per-item via their own `authorId`.
...
- **Billing plan:** Requires Blaze (pay-as-you-go), not Spark (free) — as of Feb 2026, Cloud Storage for Firebase needs a linked billing account to create a bucket at all, and both the Algolia sync extension and Firebase App Hosting (if chosen) run on Cloud Functions/Cloud Run, which are Blaze-only. Firestore and Auth usage stay within Spark's free quotas regardless. For single-user volume, expected bill is $0/month (Storage also has its own Always Free tier on top), but set a Cloud Billing budget alert as a safety net — more relevant once Phase 7 exposes public pages to arbitrary traffic.
- **Search:** Algolia (free "Build" tier: 1M records, 10,000 searches/month — well above personal-use volume) for search/filter by name, ingredient, and tag, since Firestore doesn't support full-text/array search well. Synced via the official Firebase "Search with Algolia" extension, which mirrors the `Recipe` collection into an Algolia index on every write — no hand-written sync Cloud Function needed. Free-tier apps are deleted after 60 days of inactivity, worth a periodic keepalive if usage lapses. Once Phase 7 introduces multiple accounts, the index needs to exclude private recipes from anything queryable by a client-side search key (either sync public recipes only, or use Algolia's secured API keys with a per-request filter) — noted in Open Questions.
- **Offline:** v1 supports offline _reads_ via Firestore's built-in local cache — recipes you've viewed stay available without connectivity. Offline _writes_ (editing/creating while offline) deferred to post-MVP; low conflict risk since single-user, but queuing/sync logic adds complexity not worth it for v1.
...
- Algolia + Phase 7 privacy: decide how the search index excludes private recipes once other users exist — sync public recipes only, vs. secured/filtered Algolia API keys.
