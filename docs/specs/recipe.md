...
- **URL import:** needs a scraping approach (recipe schema.org/JSON-LD parsing where available, fallback heuristics otherwise). If a site blocks scraping or has no structured data, show a friendly error and suggest the user take a full-page screenshot as a fallback import method. Leading candidate: the Python `recipe-scrapers` package (MIT, actively maintained) — tries schema.org first, then falls back to hand-written per-site scrapers for a large list of cooking sites, and deliberately doesn't fight bot protection, which lines up with the screenshot-fallback plan above. Firebase Cloud Functions 2nd gen supports Python (3.10–3.13) alongside the existing Node.js functions, but using it means a second Cloud Functions codebase with its own dependency management, plus an extra hop (Next.js server → Python function → back) rather than staying in one runtime end to end — worth deciding deliberately when Phase 3 starts, not backing into by default.
...
- ~~Ingredient string parsing~~ — resolved: a Python Cloud Function (`functions-python/`, second codebase alongside the existing Node `functions/`) wraps `ingredient-parser` (ML/CRF-based, strangetom/ingredient-parser), called via `httpsCallable` from the manual-entry flow (see [create-recipe-from-form.md](../flows/create-recipe-from-form.md)). Chosen over a client-side regex parser (`parse-ingredient`) because it gives a native per-field confidence score — exactly what the flagged/dismiss UX needs — and separates `preparation` from `name`, which the regex option couldn't do. Handles fractions and ranges; compound units ("1 lb 2 oz") are detected but always flagged rather than parsed, since combining them into a single amount is still out of scope. Also sets precedent for Phase 3's URL-scraped `recipeIngredient` strings to reuse the same server-side parser rather than a second, divergent implementation — and validates the "Python Cloud Function" direction called out in this doc's URL-import Technical Consideration above.
