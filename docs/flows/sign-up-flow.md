...
`pnpm seed` creates test Auth users with their `profiles`/`handles` docs already written, bypassing this entire flow — no manual sign-up or handle claim needed just to get accounts to sign in with locally. See [specs/seed-script.md](../specs/seed-script.md).
